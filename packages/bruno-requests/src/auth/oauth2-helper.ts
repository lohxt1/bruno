...
import qs from 'qs';
import { T_ClientCredentialsGrantTypeConfig, T_ClientCredentialsGrantTypeRequestData, T_OAuth2Config, T_PasswordGrantTypeConfig, T_PasswordGrantTypeRequestData, T_RequestConfig, T_TokenStore } from './types/oaut2-helper';

/**
 * Fetches an OAuth2 token using client credentials grant
 */
const fetchTokenClientCredentials = async (clientCredentialsConfig: T_ClientCredentialsGrantTypeConfig) => {
  const {
    accessTokenUrl,
    clientId,
    clientSecret,
  ...
  const data: T_ClientCredentialsGrantTypeRequestData = {
 ...
 */
const fetchTokenPassword = async (oauth2Config: T_PasswordGrantTypeConfig) => {
  const {
    accessTokenUrl,
    clientId,
    clientSecret,
  ...
  const data: T_PasswordGrantTypeRequestData = {
    grant_type: 'password',
    username,
    password,
    scope: scope || ''
 ...
 */
export const getOAuth2Token = async (oauth2Config: T_OAuth2Config, tokenStore: T_TokenStore): Promise<string | null> => {
